...
const TryHardTracker = forwardRef(({ startDate, endDate }, ref) => {
  ...
  return (
    <React.Fragment>
      <div style={{ position: 'fixed', bottom: 0, width: '100%', background: '#ffffff', marginLeft: "200px", height: "80px" }}>
        <Row className="mt-4">
          <Col >
          ...
          <Col>
            <Button variant="primary" className="mr-2" onClick={() => handleDecrement('good')}>
              -
            </Button>
            {" Good: "} {sessionData.goodCount || 0} {" "}
            <Button variant="primary" className="ml-2" onClick={() => handleIncrement('good')}>
              +
            </Button>
          </Col>
        </Row>
