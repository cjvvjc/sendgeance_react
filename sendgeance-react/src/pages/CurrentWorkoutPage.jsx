...
const CurrentWorkoutPage = () => {
  ...
  const totalSessionTimeInMinutes = useCallback(() => {
    const sessionTimes = currentWorkout.map(
      (workout) => new Date(workout.createdAt)
    );
  ...
  const totalBoulderingSessionTimeInMinutes = useCallback(() => {
    ...
    for (let i = 1; i < boulderingSessionTimes.length; i++) {
      ...
      const diffInMilliseconds = endTime - startTime;
      totalBoulderingSessionTime += Math.round(
  ...
  useEffect(() => {
    const calculateTotalAttempts = () => {
      const totalAttempts = currentWorkout.reduce((sum, workout) => {
        const attemptsValue =
          workout.attempts === 'Flash'
            ? 1
            : parseInt(workout.attempts, 10) || 0;
    ...
    const calculateSentCardsCount = () => {
      const sentCards = currentWorkout.filter(
        (workout) => workout.send === true
      );
  ...
  useEffect(() => {
    const fetchWorkout = async () => {
      try {
        ...
        const res = await axios.get(`http://localhost:5000/workout/current`, {
          params,
        });
        const newCurrentWorkout = res.data;

        if (
          JSON.stringify(currentWorkout) !== JSON.stringify(newCurrentWorkout)
        ) {
        ...
        const vPointAvg =
          totalProblems > 0
            ? Math.round((vPointSum / totalProblems) * 10) / 10
            : 0;
        updateSession((prevSessionData) => ({
          ...prevSessionData,
          vPointAvg,
        ...
        const sessionDensityValue =
          sessionTimeInMinutes > 0
            ? (vPointSum / sessionTimeInMinutes).toFixed(2)
            : 0;
        updateSession((prevSessionData) => ({
          ...prevSessionData,
          sessionDensityValue,
        ...
        if (boulderingSessionTimes.length > 0) {
          const boulderingStartTime = new Date(
            Math.min(...boulderingSessionTimes)
          );
          const boulderingEndTime = new Date(
            Math.max(...boulderingSessionTimes)
          );
          setSessionTimes({
            startTime: boulderingStartTime,
            endTime: boulderingEndTime,
          });
        } else {
          setSessionTimes({ startTime: null, endTime: null });
        }
        ...
        const boulderingProblems = currentWorkout.filter(
          (workout) => workout.exerciseGroup === 'Bouldering'
        );
        setTotalBoulderingProblems(boulderingProblems.length);

        const vPointBoulderingSum = boulderingProblems.reduce(
          (sum, workout) => {
            return sum + parseInt(workout.grade.substring(1), 10);
          },
          0
        ...
        const boulderingSessionTimeInMinutes =
          totalBoulderingSessionTimeInMinutes();
        setBoulderingSessionTime(boulderingSessionTimeInMinutes);

        const boulderingProblemSessionDensity =
          boulderingSessionTimeInMinutes > 0
            ? (vPointBoulderingSum / boulderingSessionTimeInMinutes).toFixed(2)
            : 0;
        updateSession((prevSessionData) => ({
          ...prevSessionData,
        ...
        <Row className="mt-4">
          <Col>
            <p>
              ...
              {sessionData.rateOfPerceivedExertion !== null
                ? sessionData.rateOfPerceivedExertion
                : 'Select an RPE'}
            </p>
            <Dropdown>
              ...
              <Dropdown.Menu>
                {[...Array(10)].map((_, index) => (
                  <Dropdown.Item
                    key={index + 1}
                    onClick={() => handleRpeSelect(index + 1)}
                  >
                    {index + 1}
                  </Dropdown.Item>
                ))}
        ...
        <Row className="mt-2 mb-2">
          <Col xs={12}>
            <p
              ...
              onClick={toggleSessionTimes}
            >
              {showSessionTimes ? '▲' : '▼'} Total Session Time:{' '}
              {totalSessionTimeInMinutes()} mins
            </p>
            {showSessionTimes && (
              <div>
                <p style={{ marginLeft: '2em', marginBottom: '0' }}>
                  ...
                  {sessionTimes.startTime
                    ? sessionTimes.startTime.toLocaleString()
                    : 'N/A'}
                </p>
                <p style={{ marginLeft: '2em', marginBottom: '0' }}>
                  Session End Time:{' '}
                  {sessionTimes.endTime
                    ? sessionTimes.endTime.toLocaleString()
                    : 'N/A'}
                </p>
              </div>
            )}
            ...
            <p style={{ marginBottom: '0' }}>
              Total Problems: {sessionData.totalProblems}
            </p>
            ...
            >
              {showSessionDensity ? '▲' : '▼'} Session Density:{' '}
              {sessionData.boulderingSessionDensity}
            </p>
            {showSessionDensity && (
              <div>
                <p style={{ marginLeft: '2em', marginBottom: '0' }}>
